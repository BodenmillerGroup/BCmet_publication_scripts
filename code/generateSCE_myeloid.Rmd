...
```{r read-in-data}
input_folder <- "~/projects/BCmet/workflowr_projects/BCmet_macrophage_wflow/data/"
...
panel <- read.csv(file = paste0(input_folder, "panel_BCmet_macrophage.csv"), stringsAsFactors = FALSE)
```
...
meta <- read.csv("data/macrophage_images_patient_metadata_encoded.csv")
```
...
tags <- read.csv("data/20200708_ZTMA_265_macrophage_rows_01_05_s0_a1_ac_full.csv", header = FALSE)
...
VEGF: bad staining quality
CCR2: specificity unclear

```{r remove-empty-channel}
sce_test <- sce[!(rownames(sce) %in% c("VEGF", "CCR2")),]
```

# Save SCE object

```{r save-RDS}
saveRDS(sce, "output/SCEs/myeloid/myeloidSCE_initial.rds")
```
